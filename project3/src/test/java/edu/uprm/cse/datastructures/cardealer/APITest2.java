...
public class APITest2 {
	...
	private Response updateCar(Car car) {
		return target.path("cars/" + car.getCarId() + "/update").request(MediaType.APPLICATION_JSON)
	...
	private Response getGetCarResponse(Car car) {
		return target.path("cars/" + car.getCarId()).request(MediaType.APPLICATION_JSON).get();
	}

	private Response deleteCar(Car car) {
		return target.path("cars/" + car.getCarId() + "/delete").request(MediaType.APPLICATION_JSON).delete();
	}

	/**
	 * Test to see that Server can insert cars in order and return all or one
	 * specific car.
	 */
	@Test
